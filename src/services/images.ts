...
export const getTrendingSpotImages = async (
  ...
  const query =
    weatherQueries[weatherCondition] || `${spotId} kitesurfing beach`;
  ...
  try {
    if (!PEXELS_API_KEY) {
      ...
      const fallbacks =
        WEATHER_FALLBACK_IMAGES[
          weatherCondition as keyof typeof WEATHER_FALLBACK_IMAGES
        ] || FALLBACK_IMAGES;
  ...
  if (uniqueImages.length < count) {
    for (let i = 0; i < images.length && uniqueImages.length < count; i++) {
      ...
      const modifiedImage = {
        ...
        url: `${image.url}${
          image.url.includes("?") ? "&" : "?"
        }t=${Date.now()}-${uniqueImages.length}`,
      };
