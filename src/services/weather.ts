...
const getRegionalFallbackData = (lat: number, lng: number) => {
  ...
  const windSpeed =
    Math.round((baseWindSpeed + (Math.random() * 4 - 2)) * 10) / 10;
  ...
  return {
    ...
    windSpeed: windSpeed,
    icon: weatherIcon,
    description: weatherIcon.includes("01")
      ? "Clear sky"
